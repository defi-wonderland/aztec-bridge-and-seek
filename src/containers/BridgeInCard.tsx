...
const ORDER_DATA_TYPE = '0xf00c3bf60c73eb97097f1c9835537da014e0b755fe94b25d7ac8401df66716a0' as const;
const BASE_SEPOLIA_CHAIN_ID = 84532;

function pad32(hex: `0x${string}`) {
  return (hex.length === 66 ? hex : (('0x' + hex.slice(2).padStart(64, '0')) as `0x${string}`));
}
...
export const BridgeInCard: React.FC = () => {
  ...
  const [tokenAddress, setTokenAddress] = useState<string>(WETH_ON_BASE_SEPOLIA);
  const [amount, setAmount] = useState<string>('');
  const [decimals, setDecimals] = useState<number>(18);
  const [balance, setBalance] = useState<bigint | null>(null);
  const [isLoading, setIsLoading] = useState(false);

  const provider = (globalThis as any).ethereum ? new ethers.BrowserProvider((globalThis as any).ethereum) : null;
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        await ((provider as any).provider?.request?.({ method: 'eth_requestAccounts' }) ?? (provider as any).send('eth_requestAccounts', []));
  ...
  useEffect(() => {
    const fetchTokenInfo = async () => {
      ...
      try {
        const erc20 = new ethers.Contract(tokenAddress, ["function decimals() view returns (uint8)"], await provider.getSigner());
  ...
  const refreshBalance = async () => {
    if (!provider || !ethers.isAddress(tokenAddress) || !ethers.isAddress(evmAddress)) return;
    const net = await provider.getNetwork();
    if (Number(net.chainId) !== BASE_SEPOLIA_CHAIN_ID) {
      addError({ message: 'Please switch your wallet to Base Sepolia to fetch balance.', type: 'error', source: 'bridge' });
      return;
    }
    try {
      const erc20 = new ethers.Contract(tokenAddress, ["function balanceOf(address) view returns (uint256)"], await provider.getSigner());
      const bal = await erc20.balanceOf(evmAddress);
      setBalance(BigInt(bal.toString()));
    } catch (err) {
      addError({ message: err instanceof Error ? err.message : 'Failed to fetch EVM balance', type: 'error', source: 'bridge' });
    }
  };
  ...
  const openOrder = async () => {
    if (!provider || !ethers.isAddress(tokenAddress) || !connectedAccount) return;
    try {
      setIsLoading(true);
      const net = await provider.getNetwork();
      if (Number(net.chainId) !== BASE_SEPOLIA_CHAIN_ID) {
        try {
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          await (provider as any).send('wallet_switchEthereumChain', [{ chainId: ethers.toBeHex(BASE_SEPOLIA_CHAIN_ID) }]);
        } catch (switchErr: any) {
          if (switchErr?.code === 4902) {
            ...
            addError({ message: 'Please switch your wallet to Base Sepolia.', type: 'error', source: 'bridge' });
            setIsLoading(false);
            return;
          }
        }
      ...
      const signer = await provider.getSigner();
      const erc20 = new ethers.Contract(tokenAddress, ["function approve(address,uint256) returns (bool)"], signer);
      ...
      const aztecRecipient = connectedAccount.getAddress().toString() as `0x${string}`;
      const recipient = aztecRecipient;
      const inputToken = padHex32(ethers.getAddress(tokenAddress) as `0x${string}`);
      ...
      const destinationSettler = '0x1b4f272b622a493184f6fbb83fc7631f1ce9bad68d4d4c150dc55eed5f100d73' as const;
      ...
      const gateway = new ethers.Contract(L2_GATEWAY_7683_ADDRESS, l2Gateway7683Abi, signer);
      const tx = await gateway.open({ fillDeadline, orderDataType: ORDER_DATA_TYPE, orderData });
      await tx.wait();

      addError({ message: 'Order opened. A filler will complete bridging to Aztec.', type: 'info', source: 'bridge' });
      await refreshBalance();
      setAmount('');
    } catch (err) {
      addError({ message: err instanceof Error ? err.message : 'Failed to open order', type: 'error', source: 'bridge' });
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="dripper-content">
      ...
      <div className="mint-form-container">
        <div className="form-section">
          ...
          <button className="btn btn-primary" onClick={openOrder} disabled={!amount || !ethers.isAddress(tokenAddress) || !provider || isLoading}>
