...
// Export a function to initialize the vanilla JS logic
export function initializeVanillaJS() {
  // DOM Elements
  ...
  const deploymentSalt = process.env.DEPLOYMENT_SALT;
  ...
  voteButton.addEventListener('click', async (e) => {
    ...
    if (!contractAddress) {
      displayError('Contract address not available');
      return;
    }
